app.factory('TaskFactory',['$http','$find','$valid',function($http,$find,$valid) {
	...
	factory.get = function(callback) {
		if (typeof(callback) == 'function') {
			if (content[0]) {
				...
				$http.get('/tasks').then(function(returned) {
					content = returned.data.tasks
					return callback(content)
				})
			}
		} else if (!callback) {
	...
	factory.create = function(new_task) {
		if ($valid.ate(factory,new_task).valid) {
			var promise = $http.post('/tasks',new_task)
			promise.then(function(returned) {
	...
	factory.update = function(id,patch) {
		if ($valid.ate(patch).valid) {
			var promise = $http.put('/tasks/'+id,{'query':{'_id':id},'patch':patch})
			promise.then(function(returned) {
				if (returned.status == 200) {
	...
	factory.delete = function(id) {
		if ($find.index(content,id)+1) {
			var promise = $http.delete('/tasks/'+id)
			promise.then(function(returned) {
				if (returned.status == 200) {
					var index = $find.index(content,id)
